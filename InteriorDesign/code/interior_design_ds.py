...
def load_models(): 
  ...
  pipe = StableDiffusionControlNetPipeline.from_pretrained( "Lykon/dreamshaper-8", controlnet=controlnet, torch_dtype=torch.float16, safety_checker=None ).to("cuda") 
  pipe.scheduler = UniPCMultistepScheduler.from_config(pipe.scheduler.config) 
  pipe.enable_attention_slicing(1) 
  return depth_estimator, pipe 
  depth_estimator, pipe = load_models() 
  prompt_dict = { "Master Bedroom":"Enhance the bedroom interior while keeping all existing furniture and layout unchanged. Maintain the current wardrobe on the wall behind the bed, the window with metal grills on the right side, and the wall-mounted AC unit above the window. Keep the carved wooden bed in the same position. Improve the room by changing the bedsheet to a neatly spread pastel-colored or earthy-tone fabric with matching pillows. Add a soft rug under the bed. Replace the curtains with modern, semi-sheer ones that allow natural light. Slightly 19 brighten the wall color while keeping it neutral (off-white or light beige). Add minimal wall art or a small painting beside the AC without removing anything. Enhance lighting subtly but realistically, maintaining natural window light and ceiling lighting. Keep everything else in the same place for realism.", 
                 #"Master Bedroom": "natural modern bedroom, luxury furniture, photorealistic materials, accurate shadows, 4K detail, architectural visualization, interior design magazine quality, professional lighting, nice walls", 
                 "Children's Bedroom": "colorful children's bedroom with twin beds, playful decor, educational toys, soft pastel colors, cartoon-themed elements, study area, storage solutions, safety features,fun wallpaper, cozy atmosphere", 
  ...
  negative_prompt = """ blurry, empty, deformed, bad anatomy, unrealistic lighting, unrealistic furniture, poor texture, low quality, cluttered, unsafe elements""" 
  def calculate_metrics(orig_img, gen_img): 
    orig_gray = np.array(orig_img.convert('L')) 
    gen_gray = np.array(gen_img.convert('L')) 
    ssim_value = ssim(orig_gray, gen_gray, data_range=255, win_size=11, gaussian_weights=True) 
    ...
    if input_image is None or room_type not in prompt_dict: 
      ...
      generated_image = pipe( prompt=prompt, negative_prompt=negative_prompt, image=input_image, control_image=depth_map, num_inference_steps=35, guidance_scale=8.0, controlnet_conditioning_scale=0.85, generator=torch.Generator("cuda").manual_seed(42) ).images[0] 
      ...
      gr.Interface( 
        ...
        inputs=[ gr.Image(type="pil", label="Upload Empty Room Image"), 
                gr.Dropdown(choices=room_choices, label="Select Room Type")], 
        outputs=[ gr.Image(label="Furnished Room Output"), 
